...
For this rule, the canonical utility is **`AsyncReturnType`**: `AsyncReturnType` unwraps the return type of a function that returns a Promise.
...
```ts
// Avoid non-canonical patterns: AsyncReturnType

...
```ts
import type { ReadonlyBy } from "utility-types";
...
For this rule, the canonical utility is **`SetReadonly`**: `SetReadonly` creates a type that makes the given keys readonly.
...
```ts
// Avoid non-canonical patterns: SetReadonly
import type { ReadonlyBy } from "utility-types";

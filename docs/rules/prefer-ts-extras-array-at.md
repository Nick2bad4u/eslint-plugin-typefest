...
For this rule, the canonical helper is **`arrayAt`**: `arrayAt` is a strongly-typed version of `Array#at()` with improved tuple support.
...
```ts
// Avoid non-canonical patterns: arrayAt

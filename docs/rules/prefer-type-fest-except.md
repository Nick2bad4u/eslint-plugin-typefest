...
For this rule, the canonical utility is **`Except`**: `Except` creates a type from an object type without certain keys and is a stricter version of `Omit`.
...
```ts
// Avoid non-canonical patterns: Except

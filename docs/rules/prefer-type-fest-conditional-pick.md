...
```ts
import type { PickByTypes } from "utility-types";
...
For this rule, the canonical utility is **`ConditionalPick`**: `ConditionalPick` is like `Pick`, but selects properties where values extend a condition type.
...
```ts
// Avoid non-canonical patterns: ConditionalPick
import type { PickByTypes } from "utility-types";

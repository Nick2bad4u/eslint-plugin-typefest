...
For this rule, the canonical helper is **`arrayFindLastIndex`**: `arrayFindLastIndex` is a strongly-typed version of `Array#findLastIndex()`.
...
```ts
// Avoid non-canonical patterns: arrayFindLastIndex

...
For this rule, the canonical utility is **`ValueOf`**: `ValueOf` creates a union of an object's values (optionally restricted to specific keys).
...
```ts
// Avoid non-canonical patterns: ValueOf

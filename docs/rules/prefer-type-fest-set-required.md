...
```ts
import type { RequiredBy } from "utility-types";
...
For this rule, the canonical utility is **`SetRequired`**: `SetRequired` creates a type that makes the given keys required.
...
```ts
// Avoid non-canonical patterns: SetRequired
import type { RequiredBy } from "utility-types";

...
For this rule, the canonical helper is **`arrayConcat`**: `arrayConcat` is a strongly-typed version of `Array#concat()` and handles different element types correctly.
...
```ts
// Avoid non-canonical patterns: arrayConcat

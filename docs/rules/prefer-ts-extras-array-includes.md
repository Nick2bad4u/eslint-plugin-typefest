...
For this rule, the canonical helper is **`arrayIncludes`**: `arrayIncludes` is a strongly-typed version of `Array#includes()` that acts as a type guard.
...
```ts
// Avoid non-canonical patterns: arrayIncludes

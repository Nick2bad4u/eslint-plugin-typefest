...
For this rule, the canonical utility is **`UnwrapTagged`**: `UnwrapTagged` gets the untagged portion of a type created with `Tagged`.
...
```ts
// Avoid non-canonical patterns: UnwrapTagged
import type { UnwrapOpaque } from "type-fest";

...
For this rule, the canonical helper is **`arrayJoin`**: `arrayJoin` is a strongly-typed version of `Array#join()` that preserves literal string types.
...
```ts
// Avoid non-canonical patterns: arrayJoin

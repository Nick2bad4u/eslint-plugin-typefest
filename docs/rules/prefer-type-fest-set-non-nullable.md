...
For this rule, the canonical utility is **`SetNonNullable`**: `SetNonNullable` creates a type that makes the given keys non-nullable.
...
```ts
// Avoid non-canonical patterns: SetNonNullable
import type { NonNullableBy } from "utility-types";

...
`isPresent` expresses nullish intent directly and provides consistent `NonNullable<T>` narrowing without repeating inline null/undefined combinations.
...
For this rule, the canonical helper is **`isPresent`**: `isPresent` checks whether a value is present (not `null` and not `undefined`).
...
```ts
// Avoid non-canonical patterns: isPresent
if (profile != null) {
    render(profile);
}
```

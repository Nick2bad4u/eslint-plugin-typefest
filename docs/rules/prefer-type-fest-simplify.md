...
For this rule, the canonical utility is **`Simplify`**: `Simplify` is useful to flatten type output to improve hints shown in editors and to aid assignability.
...
```ts
// Avoid non-canonical patterns: Simplify

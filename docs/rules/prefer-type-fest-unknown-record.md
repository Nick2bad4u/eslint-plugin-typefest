...
For this rule, the canonical utility is **`UnknownRecord`**: `UnknownRecord` represents an object with unknown values and is preferred over `{}` for unknown objects.
...
```ts
// Avoid non-canonical patterns: UnknownRecord

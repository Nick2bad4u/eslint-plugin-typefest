...
For this rule, the canonical helper is **`stringSplit`**: `stringSplit` is a strongly-typed version of `String#split()` that can return tuples for literal strings.
...
```ts
// Avoid non-canonical patterns: stringSplit

...
`isDefined` gives a reusable, named predicate with consistent narrowing behavior and avoids repeating ad-hoc undefined comparison logic.
...
For this rule, the canonical helper is **`isDefined`**: `isDefined` checks whether a value is defined (not `undefined`).
...
```ts
// Avoid non-canonical patterns: isDefined

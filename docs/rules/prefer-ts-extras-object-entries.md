...
For this rule, the canonical helper is **`objectEntries`**: `objectEntries` is a strongly-typed version of `Object.entries()`.
...
```ts
// Avoid non-canonical patterns: objectEntries

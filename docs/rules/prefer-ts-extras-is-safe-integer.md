...
For this rule, the canonical helper is **`isSafeInteger`**: `isSafeInteger` is a strongly-typed version of `Number.isSafeInteger()`.
...
```ts
// Avoid non-canonical patterns: isSafeInteger

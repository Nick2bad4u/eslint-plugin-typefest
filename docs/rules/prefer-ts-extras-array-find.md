...
For this rule, the canonical helper is **`arrayFind`**: `arrayFind` is a strongly-typed version of `Array#find()` that preserves inference better.
...
```ts
// Avoid non-canonical patterns: arrayFind

...
For this rule, the canonical utility is **`Promisable`**: `Promisable` creates a type that represents either a value or the value wrapped in `PromiseLike`.
...
```ts
// Avoid non-canonical patterns: Promisable

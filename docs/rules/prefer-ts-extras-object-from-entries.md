...
For this rule, the canonical helper is **`objectFromEntries`**: `objectFromEntries` is a strongly-typed version of `Object.fromEntries()`.
...
```ts
// Avoid non-canonical patterns: objectFromEntries

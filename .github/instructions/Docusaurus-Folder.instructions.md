...
- Website and build setup:
  ...
  - If a Docusaurus build fails because a `developer/api/...` sidebar id no longer exists, regenerate the API docs using the repository's API-doc generation command, inspect the current generated ids, and update `docs/docusaurus/sidebars.ts`. Do not create placeholder docs under the generated API folder to satisfy stale sidebar entries.
